...
import { getChatbotContext } from "./vector-storage";
import { formatTasksForChatbot, getAsanaProjectTasks } from "./asana";

/**
 * Checks if a request is from a test page
 */
export function isFromTestPage(req: Request): boolean {
...
/**
 * Validates chatbot access token and handles test page exceptions
 */
export function validateChatbotAccess(req: Request, chatbot: any, token: string): boolean {
  // Test pages get automatic access
  if (isFromTestPage(req)) {
    console.log(`Bypassing authentication for test page: ${req.headers.referer}`);
    return true;
  }
...
/**
 * Prepares system prompt and messages for OpenAI
 * This ensures consistency between streaming and non-streaming endpoints
 */
export async function prepareChatbotMessagesAndPrompt(
  chatbotId: number,
  userMessage: string,
  userId: number | null
): Promise<{
  ...
  // Add Asana to context sources if we have Asana data
  if (hasAsanaProjects) {
...
You should **never make up information**. You may summarize or synthesize details if the answer is spread across multiple sources.`;
  ...
  let systemPrompt = systemPromptTemplate
    ...
    .replace(/{{asanaNote}}/g, chatbot.asanaProjectId ? 
      "- Asana: always mention that the information comes from Asana project tasks and include the project name." : "");

  // Add custom output format if specified for the chatbot
